...
func _ready() -> void:
	...
	style_panel.border_color = Color(0.2, 0.8, 0.5, 1.0) # Verde meio Químico
	...
	lbl_titulo.text = "Cientista Maluco"
	...
	btn_cancel1.text = " X "
	btn_cancel1.pressed.connect(func(): _cancelar_quest("cientista_quest1"))
	...
	btn_cancel2.text = " X "
	btn_cancel2.pressed.connect(func(): _cancelar_quest("cientista_quest2"))
	...
	lbl_titulo.add_theme_color_override("font_color", Color(0.2, 0.8, 0.5))
...
func _estilizar_botao(btn: Button, e_cancelar: bool = false) -> void:
	...
	if e_cancelar:
		...
		sb_normal.bg_color = Color(0.15, 0.2, 0.15, 1)
		
	sb_normal.border_width_bottom = 2
	sb_normal.border_color = Color(0.2, 0.8, 0.5)
	...
	sb_hover.bg_color = Color(0.2, 0.3, 0.25, 1)
...
func _atualizar_quests() -> void:
	var q1 = PlayerStats.quests_concluidas.get("cientista_quest1", false)
	var q2 = PlayerStats.quests_concluidas.get("cientista_quest2", false)
	var ativa1 = PlayerStats.quests_ativas.get("cientista_quest1", false)
	...
	if q1 and q2:
		...
		_definir_texto_dialogo("Estou analisando os reagentes... Me avise assim que conseguir os 5 materiais de cada missão!")
	...
	if q1:
		...
		btn_quest1.text = "Aceitar Missão: Procurar Livros"
		btn_quest1.disabled = false
		btn_cancel1.hide()
	else:
		var qtd_livros = _contar_item("Livro de Fórmulas")
		btn_quest1.text = "Entregar Livros de Fórmulas (" + str(qtd_livros) + "/5)\nRecompensa: 50 Moedas"
		btn_quest1.disabled = (qtd_livros < 5)
		btn_cancel1.show()
	...
	if q2:
		...
		btn_quest2.text = "Aceitar Missão: Caçar Slimes"
		btn_quest2.disabled = false
		btn_cancel2.hide()
	else:
		var qtd_gelatina = _contar_item("Fragmento de Gelatina")
		btn_quest2.text = "Entregar Fragmentos de Gelatina (" + str(qtd_gelatina) + "/5)\nRecompensa: 50 Moedas"
		btn_quest2.disabled = (qtd_gelatina < 5)
		btn_cancel2.show()
...
func _tentar_entregar_quest1() -> void:
	if not PlayerStats.quests_ativas.get("cientista_quest1", false):
		PlayerStats.quests_ativas["cientista_quest1"] = true
		PlayerStats.salvar()
		PlayerStats.quests_atualizadas.emit()
		_atualizar_quests()
		return
		
	if not PlayerStats.quests_concluidas.get("cientista_quest1", false) and _contar_item("Livro de Fórmulas") >= 5:
		_remover_itens("Livro de Fórmulas", 5)
		PlayerStats.moedas += 50
		PlayerStats.quests_concluidas["cientista_quest1"] = true
		PlayerStats.quests_ativas["cientista_quest1"] = false
		PlayerStats.salvar()
		PlayerStats.quests_atualizadas.emit()
		_atualizar_quests()
		if not (PlayerStats.quests_concluidas.get("cientista_quest1", false) and PlayerStats.quests_concluidas.get("cientista_quest2", false)):
			_definir_texto_dialogo("Esses livros são perfeitos! Ainda faltam os materiais dos slimes para completar o experimento, mas isso já ajuda muito!")

func _tentar_entregar_quest2() -> void:
	if not PlayerStats.quests_ativas.get("cientista_quest2", false):
		PlayerStats.quests_ativas["cientista_quest2"] = true
		PlayerStats.salvar()
		PlayerStats.quests_atualizadas.emit()
		_atualizar_quests()
		return

	if not PlayerStats.quests_concluidas.get("cientista_quest2", false) and _contar_item("Fragmento de Gelatina") >= 5:
		_remover_itens("Fragmento de Gelatina", 5)
		PlayerStats.moedas += 50
		PlayerStats.quests_concluidas["cientista_quest2"] = true
		PlayerStats.quests_ativas["cientista_quest2"] = false
		PlayerStats.salvar()
		PlayerStats.quests_atualizadas.emit()
		_atualizar_quests()
		if not (PlayerStats.quests_concluidas.get("cientista_quest1", false) and PlayerStats.quests_concluidas.get("cientista_quest2", false)):
			_definir_texto_dialogo("Incrível! Essa gelatina será uma ótima base para a poção. Ainda preciso daqueles livros de fórmulas, caso encontre algum!")
