...
func coletar() -> void:
	...
	tween_coleta.tween_property(self, "scale", scale * 1.25, 0.15).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	tween_coleta.tween_property(self, "scale", Vector2.ZERO, 0.15).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
	tween_coleta.parallel().tween_property(self, "modulate:a", 0.0, 0.15)
