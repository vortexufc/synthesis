...
func _ready() -> void:
	...
	if get_node_or_null("/root/DungeonGenerator"):
		...
		print("[Sala Física] dropar_chave_no_ultimo_monstro = ", dropar_chave_no_ultimo_monstro, " para ", scene_file_path)
	...
	if count == 0:
		for child in get_children():
			if not child.is_queued_for_deletion() and not child.is_in_group("player") and child.name != "Player" and not child.name.begins_with("Player"):
				if child.name.begins_with("Robo") or child.name.begins_with("Slime") or (child is CharacterBody2D):
					count += 1
...
func _on_inimigo_derrotado(pos: Vector2) -> void:
	...
	print("[Sala Física] Inimigo derrotado. Restantes contados: %d, contador: %d" % [vivos_restantes, monstros_na_sala])
