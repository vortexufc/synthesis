...
func _ready() -> void:
	...
	if get_node_or_null("/root/DungeonGenerator"):
		...
		print("[Sala Química] dropar_chave_no_ultimo_monstro = ", dropar_chave_no_ultimo_monstro, " para ", scene_file_path)
...
func _configurar_sistema_iluminacao() -> void:
	...
	if player and not player.get_node_or_null("AuraPlayer"):
		...
		_luz_player.color = Color(1.0, 0.94, 0.82, 1.0)
		_luz_player.energy = 0.34
		_luz_player.texture_scale = 1.65
...
func _configurar_props_alquimia(tex_luz: Texture2D) -> void:
	...
	for cell in decor.get_used_cells():
		...
		# frasco verde
			...
			_luzes_props.append({"node": luz_planta, "base_energy": 0.40, "speed": 1.4, "offset": randf() * 5.0})
...
func _garantir_tochas_na_sala() -> void:
	var tochas_existentes = get_tree().get_nodes_in_group("tochas")
	var tochas_nesta_sala = 0
	for t in tochas_existentes:
		if is_ancestor_of(t):
			tochas_nesta_sala += 1
			
	if tochas_nesta_sala > 0:
		return
	...
	var cena_tocha = load("res://scenes/Entidades/Tocha.tscn")
	if not cena_tocha:
		return
	...
	var y_top = min_world.y - 40.0
	var x_left = min_world.x + 30.0
	var x_right = max_world.x - 30.0
	var y_upper = min_world.y + (max_world.y - min_world.y) * 0.35
	var y_lower = min_world.y + (max_world.y - min_world.y) * 0.75
	
	var posicoes_tochas = [
		Vector2(min_world.x + (max_world.x - min_world.x) * 0.28, y_top),
		Vector2(min_world.x + (max_world.x - min_world.x) * 0.72, y_top),
		Vector2(x_left, y_upper),
		Vector2(x_left, y_lower),
		Vector2(x_right, y_upper),
		Vector2(x_right, y_lower),
	]
	
	for i in range(posicoes_tochas.size()):
		var tocha = cena_tocha.instantiate()
		tocha.name = "TochaAuto_" + str(i + 1)
		tocha.position = posicoes_tochas[i]
		add_child(tocha)
	...
	if count == 0:
		for child in get_children():
			if not child.is_queued_for_deletion() and not child.is_in_group("player") and child.name != "Player" and not child.name.begins_with("Player"):
				if child.name.begins_with("Slime") or child.name.begins_with("Robo") or (child is CharacterBody2D):
					count += 1
...
func _on_inimigo_derrotado(pos: Vector2) -> void:
	...
	print("[Sala Química] Inimigo derrotado. Restantes contados: %d, contador: %d" % [vivos_restantes, monstros_na_sala])
...
func _dropar_chave(pos: Vector2) -> void:
	...
	print("[Sala Química] Chave dropada com sucesso na posição: ", pos)
